...
public class AlertDialogCustom extends Dialog {
	...
	public static class Builder { 
		...
		private CharSequence[] m_itemsSingleselector;
		private int m_defaultSelectItem;
		...
		public Builder setSingleSelectosItems(CharSequence[] items,int nDefaultSelectItem,final OnClickListener listener){
			m_itemsSingleselector = items;
			m_defaultSelectItem = nDefaultSelectItem;
			m_itemsSingleSelectorListener = listener;
			return this;
		}
		...
		private void createSingleSelectorItems(View layout){
			if(m_itemsSingleselector != null){
				ListView singleSelectorListView = (ListView)layout.findViewById(R.id.itemsListView);				
				ItemsSingleSelectorListAdapter adapter = new ItemsSingleSelectorListAdapter(m_dialog.getContext());
				singleSelectorListView.setAdapter(adapter);
				singleSelectorListView.setSelection(m_defaultSelectItem);
			}
		}
		...
		public AlertDialogCustom create() {  
			...
			createItems(layout);
			createSingleSelectorItems(layout);
		...
		private class ItemsSingleSelectorListAdapter extends BaseAdapter{
		    private LayoutInflater m_singleSelectorInflater;
						
			public ItemsSingleSelectorListAdapter(Context context){
				this.m_singleSelectorInflater = LayoutInflater.from(context);
			}
			
			public int getCount() {
				return m_itemsSingleselector.length;
			}
			...
			public View getView(final int position, View convertView, ViewGroup parent) {
				...
				if (convertView == null) {	
					holder=new ViewHolder();  
					convertView = m_singleSelectorInflater.inflate(R.layout.alert_dialog_custom_singleselector_items_adapter, null);
					holder.textitem = (TextView)convertView.findViewById(R.id.single_selector_item_textview);
					holder.radioitem = (RadioButton)convertView.findViewById(R.id.single_selector_item_button);
					convertView.setTag(holder);
				...
				final String strItemText = (String)m_itemsSingleselector[position];
				holder.textitem.setText(strItemText);
				RadioButton radioBtnView = (RadioButton)convertView.findViewById(R.id.single_selector_item_button);
				if(m_defaultSelectItem == position){
					radioBtnView.setChecked(true);					
				}
				else{
					radioBtnView.setChecked(false);
				}
				
				radioBtnView.setOnClickListener(new View.OnClickListener() {					
					public void onClick(View v) {
						m_itemsSingleSelectorListener.onClick(m_dialog, position);						
					}
				});
				
				convertView.setOnClickListener(new View.OnClickListener() {					
					public void onClick(View v) {
						RadioButton radioBtnView = (RadioButton)v.findViewById(R.id.single_selector_item_button);	
						radioBtnView.setChecked(true);
						m_itemsSingleSelectorListener.onClick(m_dialog, position);							
					}
				});
